...
SmartCrop 是把 CVPR 2026 研究成果 Venus 落成可演示产品的桌面端应用：用户上传一张图片并选择场景与成片比例，系统按三种构图偏好分别生成建议；用户选择方向、精修裁剪框，并下载原分辨率成片和结构化方案。报告可由 DeepSeek 只翻译文本；翻译不可用时保留英文原文，不影响裁剪结果。
...
当前产品范围见 [P0.5 人机协同构图助手](docs/product/p0-5-human-guided-composition.md)，进程边界见 [ADR-0001](docs/decisions/0001-process-boundaries.md)，人工选择决策见 [ADR-0006](docs/decisions/0006-human-selection-over-final-review.md)。
